...
#include "aodv-jf-routing-protocol.h"
#include "ns3/log.h"
...
namespace aodv
{
NS_OBJECT_ENSURE_REGISTERED (JFRoutingProtocol)
  ;

/// UDP Port for AODV control traffic
const uint32_t JFRoutingProtocol::AODV_PORT = 654;
...
//-----------------------------------------------------------------------------
JFRoutingProtocol::JFRoutingProtocol () :
  RreqRetries (2),
  RreqRateLimit (10),
  RerrRateLimit (10),
...
{
  m_nb.SetCallback (MakeCallback (&JFRoutingProtocol::SendRerrWhenBreaksLinkToNextHop, this));
}

TypeId
JFRoutingProtocol::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::aodv::JFRoutingProtocol")
    .SetParent<Ipv4RoutingProtocol> ()
    .AddConstructor<JFRoutingProtocol> ()
    .AddAttribute ("HelloInterval", "HELLO messages emission interval.",
                   TimeValue (Seconds (1)),
                   MakeTimeAccessor (&JFRoutingProtocol::HelloInterval),
                   MakeTimeChecker ())
    .AddAttribute ("RreqRetries", "Maximum number of retransmissions of RREQ to discover a route",
                   UintegerValue (2),
                   MakeUintegerAccessor (&JFRoutingProtocol::RreqRetries),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("RreqRateLimit", "Maximum number of RREQ per second.",
                   UintegerValue (10),
                   MakeUintegerAccessor (&JFRoutingProtocol::RreqRateLimit),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("RerrRateLimit", "Maximum number of RERR per second.",
                   UintegerValue (10),
                   MakeUintegerAccessor (&JFRoutingProtocol::RerrRateLimit),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("NodeTraversalTime", "Conservative estimate of the average one hop traversal time for packets and should include "
                   "queuing delays, interrupt processing times and transfer times.",
                   TimeValue (MilliSeconds (40)),
                   MakeTimeAccessor (&JFRoutingProtocol::NodeTraversalTime),
                   MakeTimeChecker ())
    .AddAttribute ("NextHopWait", "Period of our waiting for the neighbour's RREP_ACK = 10 ms + NodeTraversalTime",
                   TimeValue (MilliSeconds (50)),
                   MakeTimeAccessor (&JFRoutingProtocol::NextHopWait),
                   MakeTimeChecker ())
    .AddAttribute ("ActiveRouteTimeout", "Period of time during which the route is considered to be valid",
                   TimeValue (Seconds (3)),
                   MakeTimeAccessor (&JFRoutingProtocol::ActiveRouteTimeout),
                   MakeTimeChecker ())
    ...
    .AddAttribute ("DeletePeriod", "DeletePeriod is intended to provide an upper bound on the time for which an upstream node A "
                   ...
                   MakeTimeAccessor (&JFRoutingProtocol::DeletePeriod),
                   MakeTimeChecker ())
    .AddAttribute ("NetDiameter", "Net diameter measures the maximum possible number of hops between two nodes in the network",
                   UintegerValue (35),
                   MakeUintegerAccessor (&JFRoutingProtocol::NetDiameter),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("NetTraversalTime", "Estimate of the average net traversal time = 2 * NodeTraversalTime * NetDiameter",
                   TimeValue (Seconds (2.8)),
                   MakeTimeAccessor (&JFRoutingProtocol::NetTraversalTime),
                   MakeTimeChecker ())
    .AddAttribute ("PathDiscoveryTime", "Estimate of maximum time needed to find route in network = 2 * NetTraversalTime",
                   TimeValue (Seconds (5.6)),
                   MakeTimeAccessor (&JFRoutingProtocol::PathDiscoveryTime),
                   MakeTimeChecker ())
    .AddAttribute ("MaxQueueLen", "Maximum number of packets that we allow a routing protocol to buffer.",
                   UintegerValue (64),
                   MakeUintegerAccessor (&JFRoutingProtocol::SetMaxQueueLen,
                                         &JFRoutingProtocol::GetMaxQueueLen),
                   MakeUintegerChecker<uint32_t> ())
    .AddAttribute ("MaxQueueTime", "Maximum time packets can be queued (in seconds)",
                   TimeValue (Seconds (30)),
                   MakeTimeAccessor (&JFRoutingProtocol::SetMaxQueueTime,
                                     &JFRoutingProtocol::GetMaxQueueTime),
                   MakeTimeChecker ())
    .AddAttribute ("AllowedHelloLoss", "Number of hello messages which may be loss for valid link.",
                   UintegerValue (2),
                   MakeUintegerAccessor (&JFRoutingProtocol::AllowedHelloLoss),
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("GratuitousReply", "Indicates whether a gratuitous RREP should be unicast to the node originated route discovery.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&JFRoutingProtocol::SetGratuitousReplyFlag,
                                        &JFRoutingProtocol::GetGratuitousReplyFlag),
                   MakeBooleanChecker ())
    .AddAttribute ("DestinationOnly", "Indicates only the destination may respond to this RREQ.",
                   BooleanValue (false),
                   MakeBooleanAccessor (&JFRoutingProtocol::SetDesinationOnlyFlag,
                                        &JFRoutingProtocol::GetDesinationOnlyFlag),
                   MakeBooleanChecker ())
    .AddAttribute ("EnableHello", "Indicates whether a hello messages enable.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&JFRoutingProtocol::SetHelloEnable,
                                        &JFRoutingProtocol::GetHelloEnable),
                   MakeBooleanChecker ())
    .AddAttribute ("EnableBroadcast", "Indicates whether a broadcast data packets forwarding enable.",
                   BooleanValue (true),
                   MakeBooleanAccessor (&JFRoutingProtocol::SetBroadcastEnable,
                                        &JFRoutingProtocol::GetBroadcastEnable),
                   MakeBooleanChecker ())
    .AddAttribute ("UniformRv",
                   "Access to the underlying UniformRandomVariable",
                   StringValue ("ns3::UniformRandomVariable"),
                   MakePointerAccessor (&JFRoutingProtocol::m_uniformRandomVariable),
                   MakePointerChecker<UniformRandomVariable> ())
    .AddAttribute ("DropProbability",
                   "JellyFish attack Drop Probability",
                   DoubleValue (0),
                   MakeDoubleAccessor (&JFRoutingProtocol::SetDropProbability,
                                       &JFRoutingProtocol::GetDropProbability),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("WindowSize",
                   "JellyFish Reorder Window Size",
                   UintegerValue (0),
                   MakeUintegerAccessor (&JFRoutingProtocol::SetWindowSize,
                                         &JFRoutingProtocol::GetWindowSize),
                   MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}
...
void
JFRoutingProtocol::SetMaxQueueLen (uint32_t len)
{
  MaxQueueLen = len;
  m_queue.SetMaxQueueLen (len);
}
void
JFRoutingProtocol::SetMaxQueueTime (Time t)
{
  MaxQueueTime = t;
  m_queue.SetQueueTimeout (t);
}
void
JFRoutingProtocol::SetDropProbability (double p)
{
  DropProbability = p;
  m_queue.SetDropProbability (p);
}
void
JFRoutingProtocol::SetWindowSize (uint32_t s)
{
  WindowSize = s;
  m_queue.SetReorderWindowSize (s);
}
...
void
JFRoutingProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> stream) const
{
...
int64_t
JFRoutingProtocol::AssignStreams (int64_t stream)
{
...
JFRoutingProtocol::Start ()
{
  ...
  m_rreqRateLimitTimer.SetFunction (&JFRoutingProtocol::RreqRateLimitTimerExpire,
                                    this);
  m_rreqRateLimitTimer.Schedule (Seconds (1));

  m_rerrRateLimitTimer.SetFunction (&JFRoutingProtocol::RerrRateLimitTimerExpire,
                                    this);
...
Ptr<Ipv4Route>
JFRoutingProtocol::RouteOutput (Ptr<Packet> p, const Ipv4Header &header,
...
void
JFRoutingProtocol::DeferredRouteOutput (Ptr<const Packet> p, const Ipv4Header & header,
                                      UnicastForwardCallback ucb, ErrorCallback ecb)
{
  NS_LOG_FUNCTION (this << p << header);
  NS_ASSERT (p != 0 && p != Ptr<Packet> ());

  JFQueueEntry newEntry (p, header, ucb, ecb);
...
bool
JFRoutingProtocol::RouteInput (Ptr<const Packet> p, const Ipv4Header &header,
...
bool
JFRoutingProtocol::Forwarding (Ptr<const Packet> p, const Ipv4Header & header,
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
  NS_LOG_FUNCTION (this);
  Ipv4Address dst = header.GetDestination ();
...
void
JFRoutingProtocol::SetIpv4 (Ptr<Ipv4> ipv4)
{
  NS_ASSERT (ipv4 != 0);
  NS_ASSERT (m_ipv4 == 0);
  ...
  if (EnableHello)
    {
      m_htimer.SetFunction (&JFRoutingProtocol::HelloTimerExpire, this);
  ...
  Simulator::ScheduleNow (&JFRoutingProtocol::Start, this);
}

void
JFRoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&JFRoutingProtocol::RecvAodv, this));
...
void
JFRoutingProtocol::NotifyInterfaceDown (uint32_t i)
{
  NS_LOG_FUNCTION (this << m_ipv4->GetAddress (i, 0).GetLocal ());
...
void
JFRoutingProtocol::NotifyAddAddress (uint32_t i, Ipv4InterfaceAddress address)
{
...
void
JFRoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
  NS_LOG_FUNCTION (this);
...
bool
JFRoutingProtocol::IsMyOwnAddress (Ipv4Address src)
{
...
Ptr<Ipv4Route>
JFRoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
...
void
JFRoutingProtocol::SendRequest (Ipv4Address dst)
{
  ...
  if (m_rreqCount == RreqRateLimit)
    {
      Simulator::Schedule (m_rreqRateLimitTimer.GetDelayLeft () + MicroSeconds (100),
                           &JFRoutingProtocol::SendRequest, this, dst);
      return;
    }
  else
    m_rreqCount++;
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
         m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      Simulator::Schedule (Time (MilliSeconds (m_uniformRandomVariable->GetInteger (0, 10))), &JFRoutingProtocol::SendTo, this, socket, packet, destination);
    }
  ScheduleRreqRetry (dst);
}

void
JFRoutingProtocol::SendTo (Ptr<Socket> socket, Ptr<Packet> packet, Ipv4Address destination)
{
    socket->SendTo (packet, 0, InetSocketAddress (destination, AODV_PORT));

}
void
JFRoutingProtocol::ScheduleRreqRetry (Ipv4Address dst)
{
  NS_LOG_FUNCTION (this << dst);
  ...
  m_addressReqTimer[dst].SetFunction (&JFRoutingProtocol::RouteRequestTimerExpire, this);
...
void
JFRoutingProtocol::RecvAodv (Ptr<Socket> socket)
{
...
bool
JFRoutingProtocol::UpdateRouteLifeTime (Ipv4Address addr, Time lifetime)
{
...
void
JFRoutingProtocol::UpdateRouteToNeighbor (Ipv4Address sender, Ipv4Address receiver)
{
...
void
JFRoutingProtocol::RecvRequest (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  NS_LOG_FUNCTION (this);
  RreqHeader rreqHeader;
...
void
JFRoutingProtocol::SendReply (RreqHeader const & rreqHeader, RoutingTableEntry const & toOrigin)
{
...
void
JFRoutingProtocol::SendReplyByIntermediateNode (RoutingTableEntry & toDst, RoutingTableEntry & toOrigin, bool gratRep)
{
  NS_LOG_FUNCTION (this);
  ...
  if (toDst.GetHop () == 1)
    {
      ...
      toNextHop.m_ackTimer.SetFunction (&JFRoutingProtocol::AckTimerExpire, this);
...
void
JFRoutingProtocol::SendReplyAck (Ipv4Address neighbor)
{
...
void
JFRoutingProtocol::RecvReply (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
...
void
JFRoutingProtocol::RecvReplyAck (Ipv4Address neighbor)
{
  NS_LOG_FUNCTION (this);
  RoutingTableEntry rt;
...
void
JFRoutingProtocol::ProcessHello (RrepHeader const & rrepHeader, Ipv4Address receiver )
{
...
void
JFRoutingProtocol::RecvError (Ptr<Packet> p, Ipv4Address src )
{
...
void
JFRoutingProtocol::RouteRequestTimerExpire (Ipv4Address dst)
{
  NS_LOG_LOGIC (this);
  RoutingTableEntry toDst;
...
void
JFRoutingProtocol::HelloTimerExpire ()
{
  NS_LOG_FUNCTION (this);
...
void
JFRoutingProtocol::RreqRateLimitTimerExpire ()
{
  NS_LOG_FUNCTION (this);
  m_rreqCount = 0;
  m_rreqRateLimitTimer.Schedule (Seconds (1));
...
void
JFRoutingProtocol::RerrRateLimitTimerExpire ()
{
  NS_LOG_FUNCTION (this);
  m_rerrCount = 0;
  m_rerrRateLimitTimer.Schedule (Seconds (1));
...
void
JFRoutingProtocol::AckTimerExpire (Ipv4Address neighbor, Time blacklistTimeout)
{
  NS_LOG_FUNCTION (this);
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      Simulator::Schedule (jitter, &JFRoutingProtocol::SendTo, this , socket, packet, destination);
    }
}

void
JFRoutingProtocol::SendPacketFromQueue (Ipv4Address dst, Ptr<Ipv4Route> route)
{
  NS_LOG_FUNCTION (this);
  JFQueueEntry JFQueueEntry;
  while (m_queue.Dequeue (dst, JFQueueEntry))
    {
      DeferredRouteOutputTag tag;
      Ptr<Packet> p = ConstCast<Packet> (JFQueueEntry.GetPacket ());
      ...
      UnicastForwardCallback ucb = JFQueueEntry.GetUnicastForwardCallback ();
      Ipv4Header header = JFQueueEntry.GetIpv4Header ();
...
void
JFRoutingProtocol::SendRerrWhenBreaksLinkToNextHop (Ipv4Address nextHop)
{
...
void
JFRoutingProtocol::SendRerrWhenNoRouteToForward (Ipv4Address dst,
...
void
JFRoutingProtocol::SendRerrMessage (Ptr<Packet> packet, std::vector<Ipv4Address> precursors)
{
  NS_LOG_FUNCTION (this);
      ...
      if (m_routingTable.LookupValidRoute (precursors.front (), toPrecursor))
        {
          ...
          Simulator::Schedule (Time (MilliSeconds (m_uniformRandomVariable->GetInteger (0, 10))), &JFRoutingProtocol::SendTo, this, socket, packet, precursors.front ());
          m_rerrCount++;
        }
      return;
    }
...
Ptr<Socket>
JFRoutingProtocol::FindSocketWithInterfaceAddress (Ipv4InterfaceAddress addr ) const
{
